...
public class BotCommand {
        ...
        dispatcher.register(
            CommandManager.literal("pvpbot")
                ...
                .then(CommandManager.literal("faction")
                    ...
                    .then(CommandManager.literal("giverandomkit")
                        .then(CommandManager.argument("faction", StringArgumentType.word())
                            .suggests(FACTION_SUGGESTIONS)
                            .then(CommandManager.argument("kitweights", StringArgumentType.greedyString())
                                .executes(ctx -> giveRandomKitToFaction(ctx.getSource(),
                                    StringArgumentType.getString(ctx, "faction"),
                                    StringArgumentType.getString(ctx, "kitweights")))
                            )
                        )
                    )
                )
    ...
    /**
     * Handles: /pvpbot faction giverandomkit {faction} 5%kit1,7%kit2,65%kit3
     *
     * Each bot in the faction independently gets a kit chosen via weighted random selection.
     * Weights do not need to sum to 100 — they are treated as relative weights.
     * Any bot whose randomly selected kit does not exist is skipped and reported.
     *
     * Format of kitweights:  <weight>%<kitname>,<weight>%<kitname>,...
     * Example:               5%archer,20%warrior,75%tank
     */
    private static int giveRandomKitToFaction(ServerCommandSource source, String factionName, String kitWeights) {
        // --- Validate faction ---
        if (!BotFaction.getAllFactions().contains(factionName)) {
            source.sendError(Text.literal("Faction '" + factionName + "' not found!"));
            return 0;
        ...
        // --- Parse weight string ---
        // Expected: "5%kit1,7%kit2,65%kit3"
        Map<String, Double> weightMap = new HashMap<>();
        String[] entries = kitWeights.split(",");
        for (String entry : entries) {
            entry = entry.trim();
            int pct = entry.indexOf('%');
            if (pct <= 0) {
                source.sendError(Text.literal(
                    "Invalid format '" + entry + "'. Expected: <weight>%<kitname>  e.g. 20%warrior"));
                return 0;
            }
            String weightStr = entry.substring(0, pct).trim();
            String kitName   = entry.substring(pct + 1).trim().toLowerCase();
            double weight;
            try {
                weight = Double.parseDouble(weightStr);
            } catch (NumberFormatException e) {
                source.sendError(Text.literal("Invalid weight '" + weightStr + "' in '" + entry + "'"));
                return 0;
            }
            if (weight <= 0) {
                source.sendError(Text.literal("Weight must be > 0, got '" + weightStr + "' in '" + entry + "'"));
                return 0;
            }
            if (!BotKits.kitExists(kitName)) {
                source.sendError(Text.literal("Kit '" + kitName + "' not found!"));
            ...
            weightMap.merge(kitName, weight, Double::sum);
        }

        if (weightMap.isEmpty()) {
            source.sendError(Text.literal("No valid kit-weight pairs found in: " + kitWeights));
            return 0;
        }

        // Build parallel lists for O(n) weighted pick
        List<String>  kitNames  = new ArrayList<>(weightMap.keySet());
        List<Double>  weights   = new ArrayList<>();
        double totalWeight = 0;
        for (String k : kitNames) {
            double w = weightMap.get(k);
            weights.add(w);
            totalWeight += w;
        }
        ...
        for (String memberName : members) {
            ...
            String chosen = kitNames.get(kitNames.size() - 1); // fallback to last
            double cumulative = 0;
            for (int i = 0; i < kitNames.size(); i++) {
                cumulative += weights.get(i);
                if (roll < cumulative) {
                    chosen = kitNames.get(i);
                    break;
                }
            }

            if (BotKits.giveKit(chosen, bot)) {
                given++;
            } else {
                skipped++;
            }
        ...
        String kitList = String.join(", ", kitNames);
        source.sendFeedback(() -> Text.literal(
            "Gave random kits [" + kitList + "] to " + finalGiven + " bots in faction '" + factionName + "'"
